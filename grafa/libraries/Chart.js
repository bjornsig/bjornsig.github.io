...
(function(){
	...
	Chart.Type.extend({
		...
		draw : function(easeDecimal){
			...
			this.chart.ctx.font = 'normal 20px Helvetica';
			...
			for(var i = 0; i < this.segments.length; i++){
				...
				if( percentage > 3 ){
					...
					this.chart.ctx.save();
					this.chart.ctx.translate(200, 200);
					this.chart.ctx.rotate(centreAngle);
					this.chart.ctx.translate(rangeFromCentre, 0);
					if (centreAngle > 7.853981) {
						this.chart.ctx.rotate(Math.PI);
					}
					this.chart.ctx.fillText(this.segments[i].label, 0, 0);
